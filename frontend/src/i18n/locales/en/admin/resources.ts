export default {
    ...
    vipTiers: {
      ...
      fields: {
        ...
        unlimitedConcurrency: 'No concurrency limit',
        unlimitedConcurrencyHint: 'Removes the ceiling entirely instead of adding to it. One holder can then occupy every free slot, so keep this for the top tier only',
        rpmLimit: 'Bonus RPM',
        rpmLimitHint: 'Extra requests per minute, added the same way. 0 grants none. Users who already have no RPM limit are left alone rather than capped',
        unlimitedRpm: 'No RPM limit',
        unlimitedRpmHint: 'Removes the requests-per-minute ceiling. Safe alongside a bounded concurrency, which still caps how much work runs at once',
