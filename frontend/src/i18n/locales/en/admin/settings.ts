export default {
    settings: {
      ...
      payment: {
        ...
        providerSepay: 'SePay (Vietnamese bank transfer)',
        providerNowpayments: 'NOWPayments (crypto)',
        sepayWebhookHint: 'Configure the following URL as the Webhook in the SePay dashboard (Company → Webhooks), with the Authorization API key set to the value above:',
        nowpaymentsWebhookHint: 'This URL is sent to NOWPayments as ipn_callback_url on every invoice. It must also be reachable from the internet:',
        ...
        field_bankCode: 'Bank code',
        field_bankCodeHint: 'The short code SePay uses for the bank, e.g. MB, VCB, TCB, ACB. If your bank is not recognised, set the Napas BIN below instead.',
        field_bankBin: 'Napas BIN',
        field_bankBinHint: 'Six-digit bank identifier used by VietQR. Only needed when the bank code above is not recognised.',
        field_accountName: 'Account holder name',
        field_sepayApiKeyHint: 'The API key configured on the SePay webhook. Requests whose Authorization header does not match are rejected.',
        field_apiToken: 'SePay user API token',
        field_sepayApiTokenHint: 'Optional. Without it an order is only confirmed by the webhook — with it, a pending order can also be reconciled by polling SePay.',
        field_ipnSecret: 'IPN secret',
        field_ipnSecretHint: 'Used to verify the HMAC signature NOWPayments sends on every IPN callback.',
        ...
        field_paymentCurrencyHint: 'NOWPayments quotes the price in this currency and the buyer pays any coin. SePay always settles in VND and ignores this setting.',
