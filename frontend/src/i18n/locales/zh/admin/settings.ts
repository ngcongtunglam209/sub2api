export default {
    settings: {
      ...
      payment: {
        ...
        providerSepay: 'SePay（越南银行转账）',
        providerNowpayments: 'NOWPayments（加密货币）',
        sepayWebhookHint: '请在 SePay 后台（Company → Webhooks）将下面的地址配置为 Webhook，并把 Authorization API key 设为上方填写的值：',
        nowpaymentsWebhookHint: '每次开票时该地址会作为 ipn_callback_url 发送给 NOWPayments，需保证公网可访问：',
        typeDisabled: '类型已禁用',
        ...
        field_accountNumber: '银行账号',
        field_bankCode: '银行代码',
        field_bankCodeHint: 'SePay 使用的银行简码，例如 MB、VCB、TCB、ACB。若银行未被识别，请改填下方的 Napas BIN。',
        field_bankBin: 'Napas BIN',
        field_bankBinHint: 'VietQR 使用的六位银行标识。仅在上方银行代码无法识别时需要填写。',
        field_accountName: '账户名',
        field_sepayApiKeyHint: 'SePay Webhook 中配置的 API Key。Authorization 头不匹配的请求会被拒绝。',
        field_apiToken: 'SePay 用户 API Token',
        field_sepayApiTokenHint: '可选。不填时订单只能由 Webhook 确认；填写后可通过轮询 SePay 对账待支付订单。',
        field_ipnSecret: 'IPN 密钥',
        field_ipnSecretHint: '用于校验 NOWPayments 每次 IPN 回调携带的 HMAC 签名。',
        field_secretKey: '密钥',
        ...
        field_paymentCurrencyHint: 'NOWPayments 以该币种计价，买家可用任意加密货币支付。SePay 固定以 VND 结算，忽略此设置。',
