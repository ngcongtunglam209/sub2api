export default {
    ...
    vipTiers: {
      ...
      fields: {
        ...
        unlimitedConcurrency: '并发不限量',
        unlimitedConcurrencyHint: '直接取消并发上限，而不是在原值上叠加。一个持有者即可占满全部空闲槽位，仅建议用于最高等级',
        rpmLimit: '赠送 RPM',
        rpmLimitHint: '每分钟请求数的赠送量，叠加方式与并发相同。0 表示不赠送。本就没有 RPM 上限的用户保持原状，不会被反向限住',
        unlimitedRpm: 'RPM 不限量',
        unlimitedRpmHint: '取消每分钟请求数上限。并发仍有上限时是安全的——真正决定同时跑多少活的是并发',
        graceDays: '有效期（天）',
