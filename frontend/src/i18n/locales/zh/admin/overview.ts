export default {
    ...
    users: {
      ...
      vipTier: {
        ...
        assign: '等级分配',
        automatic: '自动评定',
        automaticHint: '随消费升降，未续费则过期。',
        tierSummary: '倍率 {rate}x · 并发下限 {concurrency}',
        pinWarning: '锁定后不再随消费升降，也不会过期，直到解除锁定。',
        saved: 'VIP 等级已更新',
        saveFailed: 'VIP 等级更新失败',
    ...
    groups: {
      ...
      // 此前该块被误嵌在 modelRouting 内部，与 en 的层级不一致，
      // 导致中文界面直接显示 key 路径。localeParity.spec.ts 会防止再次发生。
      supportedScopes: {
        title: '支持的模型系列',
